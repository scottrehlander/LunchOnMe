<?php
	...
	class UsersTableDataAdapter extends BaseDataAdapter
	{
		...
		function AuthenticateUser($username, $password)
		{
			...
			$sql = "SELECT * FROM users WHERE upper(users.Username) = upper(:username) AND upper(users.UserPassword) = upper(:password)";
		...
		{
			$sql = "SELECT * FROM users WHERE users.UserEmail = :emailAddress";
			$varsArray = array ( ':emailAddress'=>$emailAddress );
		...
		function UpdatePassword($userId, $newPassword)
		{
			...
			$varsArray = array ( ':newPassword'=>$newPassword, ':userId'=>$userId );
		...
		function UpdateImage($userId, $newUrl)
		{
			$sql = "UPDATE users SET UserPicture = :newUrl WHERE UserId = :userId";
			$varsArray = array ( ':newUrl'=>$newUrl, ':userId'=>$userId );
		...
		function UpdateFirstAndLastName($userId, $firstName, $lastName)
		{
			$sql = "UPDATE users SET UserFirstName = :firstName, UserLastName = :lastName WHERE UserId = :userId";
			$varsArray = array ( ':firstName'=>$firstName, ':lastName'=>$lastName, ':userId'=>$userId );
		...
		function UpdateEmail($userId, $newEmail)
		{
			...
			$varsArray = array ( ':newEmail'=>$newEmail, ':userId'=>$userId );
	...
	class BillsTableAdapter extends BaseDataAdapter
	{
		...
		// Returns the billId of the bill that is the created - billConfirmed should be either 0 or 1
		function CreateBill($billAmount, $billLocation, $userBillOwedTo, $userBillOwedBy, $billNotes, $billConfirmed)
		{
			$sql = "INSERT INTO bills (BillAmount, BillLocation, UserBillOwedTo, UserBillOwedBy, BillNotes, BillConfirmed) ";
			$sql .= "VALUES (:billAmount, :billLocation, :userBillOwedTo, :userBillOwedBy, :billNotes, :billConfirmed)";
			$varsArray = array(
		...
		{
			$sql = "UPDATE bills SET BillConfirmed = 1 WHERE BillId = :billId";
			$varsArray = array ( ':billId'=>$billId );
